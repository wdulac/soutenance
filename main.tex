...
\begin{frame}[c]
    ...
    \begin{figure}
        \centering
        \includegraphics[width=\textwidth]{Figures/coverage_ratio_and_missed.png}
...
\begin{frame}[c]
    ...
    \begin{columns}[t]
        \begin{column}{0.4\textwidth}
           ...
           \onslide<4->{
               \footnotesize
               \begin{examples}[Similarité angulaire]
                   \scriptsize
                   Pour deux vecteurs issus au temps $t$ issus de trajectoires différentes :
                   %
                   \onslide<5->{
           ...
           \begin{block}
               ...
               \setlength{\leftmargini}{2.5ex}
               \begin{itemize}
                   \item<6-> Union $C \cup Q$ : Ensemble des points des deux trajectoires
                   \item<7-> \alert{Intersection} $C \cap Q$ : Co-existance des deux trajectoires aux mêmes temps $t$
                   \item<8-> \textcolor{green}{Différence symmétrique} $C \ominus Q$ : Points où une seule trajectoire est définie
               \end{itemize}
           \end{block}
       }
        \end{column}
